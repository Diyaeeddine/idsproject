...
<div id="toast-container" class="fixed top-5 right-5 z-50 space-y-4 pointer-events-none">
    ...
    <div id="toast-new"
        ...
        <button type="button" id="toast-new-close-btn"
                ...
                aria-label="Close" onclick="closeToast('toast-new')">
    ...
    <div id="toast-late"
        ...
        <button type="button" id="toast-late-close-btn"
                ...
                aria-label="Close" onclick="closeToast('toast-late')">
            <span class="sr-only">Close</span>
            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
            </svg>
...
<script>
 function closeToast(toastId) {
    ...
    toastElement.classList.add('animate-slide-out-right'); // Ajouter l'animation de fermeture

    // Supprimer le toast après l'animation de fermeture
    setTimeout(() => {
        toastElement.remove(); // Supprime l'élément du DOM
    }, 300); // 300ms pour que l'animation soit terminée
}
...
class ToastManager {
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            // Afficher les toasts manuellement pour tester
            setTimeout(() => {
