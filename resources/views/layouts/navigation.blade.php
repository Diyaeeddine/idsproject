      ...
      <div class="flex">
        <div class="shrink-0 logo-wrapper flex items-center">
          <a href="{{ route('dashboard') }}">
            <img src="{{ asset('build/assets/images/logo-marina.png') }}" class="marina-logo"/>
          </a>
        </div>
        ...
        @if(Auth::User()->role === UserRole::User)
          ...
          <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
            ...
            :active="request()->routeIs('contrats.index') || request()->routeIs('contrats.create')"
          >
            {{ __('Contrats') }}
          </x-nav-link>
          </div>
          <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
            ...
            :active="request()->routeIs('user.factures') || request()->routeIs('factures.index')"
          >
            {{ __('Factures') }}
          </x-nav-link>
          </div>
      ...
      <div class="flex items-center sm:hidden space-x-2"
        ...
        @if(Auth::User()->role === UserRole::User && isset($demandes))
          <button
            ...
            aria-expanded="false"
            aria-label="Notifications"
            class="relative inline-flex items-center p-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white focus:outline-none rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition"
            title="Notifications"
            :class="unreadCount > 0 ? 'animate-ring' : ''"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"
          ...
          <div
            ...
            x-transition:leave-start="opacity-100 translate-y-1"
            x-transition:leave-end="opacity-0 -translate-y-4"
            class="mobile-notification-dropdown max-h-80 overflow-auto rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-50"
            style="display: none;"
          >
            <div
              class="block px-4 py-2 font-semibold text-center text-gray-700 rounded-t-lg bg-gray-50 dark:bg-gray-900 dark:text-white">
  ...
  <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
    <div class="pt-2 pb-3 space-y-1 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
      ...
      @if(Auth::user()->role === UserRole::Admin)
        ...
        <x-responsive-nav-link :href="route('demande.add-demande')" :active="request()->routeIs('demande.add-demande')">
          {{ __('Création') }}
        </x-responsive-nav-link>
        <x-responsive-nav-link :href="route('demandes.affecter')" :active="request()->routeIs('demandes.affecter')">
          {{ __('Afféctation') }}
        </x-responsive-nav-link>
        <x-responsive-nav-link
          :href="route('admin.demandes')"
        ...
        <x-responsive-nav-link :href="route('budget-tables.create')" :active="request()->routeIs('budget-tables.create')">
          {{ __('Création Table Budgétaire') }}
        </x-responsive-nav-link>
        <x-responsive-nav-link :href="route('budget-tables.index')" :active="request()->routeIs('budget-tables.index') || request()->routeIs('budget-tables.show')">
          {{ __('Tables Budgétaires') }}
        </x-responsive-nav-link>
        <x-responsive-nav-link :href="route('acce.index')" :active="request()->routeIs('acce.index') || request()->routeIs('acce.edit') || request()->routeIs('acce.update') || request()->routeIs('profile.add-profile')">
          {{ __('Accés') }}
        </x-responsive-nav-link>
      @endif
      ...
      @if(Auth::User()->role === UserRole::User)
        ...
        <x-responsive-nav-link :href="route('user.demandes')" :active="request()->routeIs('user.demandes') || request()->routeIs('user.demandes.showRemplir') || request()->routeIs('user.demandes.voir')">
          {{ __('Mes demandes') }}
        </x-responsive-nav-link>
        <x-responsive-nav-link
          :href="route('contrats.index')"
